...
- **Flags pro Buchung** (unabhängige Marker, nicht nur ein einzelnes Label):
  ...
  - 🏛️ **Steuerrelevant** — fließt in den Jahres-Steuer-Export ein (siehe unten).
...
- **Projektbezogene Tags:** beliebige Hashtags (`#Urlaub2026`, `#Renovierung`, …) auf einzelnen Buchungen, editierbar im Bearbeiten-Formular (mehrere Tags per Leerzeichen getrennt). Über der Transaktionsliste werden alle bisher vergebenen Tags als anklickbare Chips angezeigt — ein Klick filtert die Liste auf genau diesen Tag und zeigt die Summe der gefilterten Buchungen an, unabhängig von der jeweiligen Kategorie (kategorieübergreifende Auswertung, z. B. "was hat der Urlaub insgesamt gekostet").
- **Steuer-Marker & Jahres-Export:** ein viertes Flag ("Steuerrelevant", Bankgebäude-Symbol) neben Vermeidbar/Ineffizient/Zu hoch, einzeln oder per Batch-Bearbeitung setzbar. "Steuer-Export"-Button über der Transaktionsliste (Jahr wählbar) lädt eine ZIP-Datei mit einer CSV aller steuerrelevanten Buchungen des gewählten Jahres (Datum, Beschreibung, Kategorie, Betrag, Tags) sowie — im Ordner `belege/` — allen in diesem Jahr hochgeladenen Belegen. Da nicht als "Wichtig" markierte Belege nach 30 Tagen automatisch gelöscht werden (siehe Rechnungen/Belege), sollten für die Steuererklärung relevante Belege beim Hochladen entsprechend markiert werden, damit sie zum Zeitpunkt des Jahres-Exports noch vorhanden sind.
...
- **Geldfluss-Diagramm (Sankey):** zeigt, wohin die Einnahmen des Zeitraums fließen — ein Einnahmen-Knoten links, verzweigt sich zu Ausgabenkategorien und "Gespart" rechts, Bandbreite proportional zum Betrag. Als handgebaute SVG-Grafik umgesetzt statt über eine Chart.js-Sankey-Bibliothek (siehe `SankeyChart.tsx`-Kommentar für den Hintergrund).
